...
from utils.io import load_config, load_category_vocabulary, load_ingredient_vocabulary, \
    load_product_name_vocabulary
from utils.metrics import evaluation_report
from utils.preprocess import get_nlp


model_path = settings.WEIGHTS_DIR / 'baseline_2' / 'best.hdf5'
...
val_df['deepest_categories'] = get_deepest_categories(category_taxonomy, val_df.categories_tags)

X_val, y_val = generate_data_partial(val_df)

y_pred_val = model.predict(X_val)

val_df['predicted_deepest_categories'] = get_deepest_categories(category_taxonomy, [
    [category_names[i] for i, conf in enumerate(y) if conf >= 0.5] for y in y_pred_val
])

val_df['is_correct'] = [source == predicted
                        for source, predicted in
                        zip(val_df.deepest_categories, val_df.predicted_deepest_categories)]

error_categories_val = [get_error_category(predicted, true, category_taxonomy)
                        for predicted, true in zip(val_df.predicted_deepest_categories,
                                                   val_df.deepest_categories)]

(val_df['missing_cat_error'], val_df['additional_cat_error'],
 val_df['over_pred_cat_error'], val_df['under_pred_cat_error']) = zip(*error_categories_val)

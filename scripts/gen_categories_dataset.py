...
class IdsSelector:
    ...
    def agribalyse_cats(self):
        ...
        try:
            ...
            self._caches["agribalyse_cats"] = value  # hack: cache
        return value
    ...
    def kind_sizes(self):
        ...
        try:
            ...
            num_agri = self.quantities["total"] * self.quantities["each_agribalyse"] / len(self.agribalyse_cats)
            ...
            self._caches["kind_sizes"] = kind_sizes  # hack: cache
        return kind_sizes
    ...
    def select_ids(self):
        ...
        def add_randomly(data, kind, weight=1):
            ...
            # and also to immediatly decide if we retain an id or not !
...
def generate_data():
    ...
    if os.path.exists("_ids_by_kind.json.gz"):
        print("Loading ids")
        ids_by_kind = json.load(gzip.open("_ids_by_kind.json.gz", "rt"))
        ids_selector.check_ids(ids_by_kind)
    else:
        print("Selecting ids")
        ids_by_kind = ids_selector.ids_by_kind()
        json.dump(ids_by_kind, gzip.open("_ids_by_kind.json.gz", "wt"))

...
dont_take = [
    'cg', 'cl', 'com', 'consommer', 'de', 'dont', 'et', 'fat', 'free', 'g',
    'ingredient', 'ingredients', 'kcal', 'kg', 'kj', 'kl', 'l', 'la', 'le',
    'mg', 'ml', 'net', 'net', 'of', 'or', 'per', 'poids', 'pour', 'total',
    'valeur', 'which', 'wt', 'www'
    ]
...
def get_best_tfidf_words_list(sub_df_idx:int, feature_names:np.ndarray, tfidf_matrix,  n_words_to_pick:int=120)->str:
    ...
    words_picked: list:
        list of N words
    """
    indexes_sorted_scores = np.argsort(tfidf_matrix[sub_df_idx].data)[-n_words_to_pick:]
    indices_to_pick = tfidf_matrix[sub_df_idx].indices[indexes_sorted_scores]
...
def get_best_tfidf_words_cleaned(sub_df_idx:int, feature_names:np.ndarray, text_series:pd.Series, tfidf_matrix, freq_dict: dict,  n_max_words_to_pick:int=120)->str:
    ...
    tfidf_matrix: scipy.sparse.csr.csr_matrix
        tfidf sparse matrix.
    freq_dict: dict
        dictionary of word frequencies in the whole dataset.
    n_max_words_to_pick: int, default = 120
        total words to pick at most, it may be less than 120 because duplicated words and infrequent words are removed 
    ...
    words_picked: list:
        list of N words
    """
    word_list = get_best_tfidf_words_list(sub_df_idx, feature_names, tfidf_matrix, n_max_words_to_pick)
    df_text = text_series.iloc[sub_df_idx]
    #this step is made to get the words original order.
...
def get_intersect_words_lang(lang_dict, n_max = 30):
    ...
    ----------
    lang_dict: dict
        dictionary in the following format {"lang": {"prob": prob, "len_text":len_text, "text": text}}
    n_max: int
        N words to take
...
def get_score_from_vertices(txt_annotations:dict):
    ...
    txt_annotations: dict
        dictionary containing detection boxes coordinates for a given image.
    ...
    vertices = txt_annotations['boundingPoly']['vertices']
...
def get_n_cleaned_words(results, word_count_limit = 10):
...
def get_big_words_from_txt_annotations(txt_annotations, word_count_limit=10):
    """ selects N unique words from an image
    Parameters
    ----------
    txt_annotations: dict
    ...
    """
    try:
        results = sorted([get_score_from_vertices(txt_a) for txt_a in txt_annotations], reverse = True)
        text = get_n_cleaned_words(results, word_count_limit = word_count_limit)
    except:
        text = ""
    return text


def get_big_words_from_image(barcode, df, keys = None, word_count_limit = 15):
    ...
    for link in links:
        ...
        text = get_big_words_from_txt_annotations(txt_annotations, word_count_limit=15)
        texts.append(text)
    return " ".join(texts)

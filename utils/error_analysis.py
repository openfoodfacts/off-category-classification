...
def get_interactive_embedding_plot(embeddings: np.ndarray,
                                   df: pd.DataFrame):
    ...
    for filter_, name, color in zip(
            ('is_correct', 'missing_cat_error', 'additional_cat_error',
...
def get_deepest_categories(taxonomy: Taxonomy,
                           categories_tags: Iterable[List[str]]) -> List[List[str]]:
    return [sorted((x.id for x in
                    taxonomy.find_deepest_nodes([taxonomy[c] for c in categories])))
            for categories in categories_tags]


def get_error_category(predicted_categories: List[str],
                       true_categories: List[str],
                       taxonomy: Taxonomy) -> Tuple[bool, bool, bool, bool]:
    (missing_nodes, additional_nodes, over_predicted_nodes,
     under_predicted_nodes) = get_classification_errors(predicted_categories,
                                                        true_categories,
                                                        taxonomy)

    return (bool(len(missing_nodes)), bool(len(additional_nodes)),
            bool(len(over_predicted_nodes)), bool(len(under_predicted_nodes)))


def get_classification_errors(predicted_categories: List[str],
                              true_categories: List[str],
                              taxonomy: Taxonomy) -> \
        Tuple[List[TaxonomyNode], List[TaxonomyNode], List[TaxonomyNode], List[TaxonomyNode]]:
    ...
    for true_category in list(true_categories_nodes):
        ...
        elif true_category.is_child_of_any(predicted_categories_nodes):
            true_categories_nodes.remove(true_category)
